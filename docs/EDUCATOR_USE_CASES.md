...
> 2026학년도 5학년 3반 2학기 과정중심평가 시스템을 Google Sheets로 만들어줘. 학생은 학생01부터 학생24까지, 교과는 국어·사회·수학·과학·영어로 구성하고 평가 수준은 매우잘함·잘함·보통·노력요함으로 해줘. 반드시 전용 과정중심평가 템플릿을 사용해서 9개 탭이 서로 연결되게 만들어줘. 입력 칸은 행 높이를 넉넉하게 하고, 긴 성취기준·관찰기록·피드백 열은 충분히 넓혀 줄바꿈해줘. 표 머리글은 고정하고 제목·입력 영역·자동 계산 영역이 한눈에 구분되게 해줘. 완료하기 전에 학생 수, 교과 수, 수식·드롭다운·차트 연결을 확인하고 결과 링크와 탭별 사용 순서를 알려줘.
...
결과물의 구조와 읽기 편의까지 지정한 긴 버전은 [공식 홈페이지의 예시 프롬프트](https://edu.jeld.kr/#prompts)에서 서비스별로 복사할 수 있습니다. 아래 문장은 기능을 빠르게 확인하는 축약 예시입니다.
...
| 새 Docs 문서와 본문 블록 생성 (표·글머리·참고 상자·A4) | 지원 |
| 교수·학습 과정안 양식 생성 (`docs_create_lesson_plan`) | 지원 |
| 학생용 학습지 양식 생성 (`docs_create_worksheet`) | 지원 |
